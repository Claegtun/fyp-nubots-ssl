...
frequencies = np.array([f_s//3, f_s//2, f_s, f_s*3//2, f_s*2, f_s*5//2])
...
Δθ_log = np.zeros((n_frequencies, n_runs))
Δφ_log = np.zeros((n_frequencies, n_runs))
...
# For each frequency, simulate the room and try to localise the source.
for i_frequency in range(n_frequencies):
    ...
    chrono = time.time()
    if i_frequency == 0:
        room = set_up_room(audio_anechoic_16, frequencies[i_frequency])
    elif i_frequency == 1:
        ...
        room = set_up_room(audio_anechoic_120, frequencies[i_frequency])
    ...
    for i_run in range(n_runs):
        ...
        # Store the logs.
        angles_log[i_run % 100, :] = np.array([i_frequency, i_run, θ, φ])
        Δθ_log[i_frequency, i_run] = Δθ
        Δφ_log[i_frequency, i_run] = Δφ

        # Log progress for every hundredth run.
        progress = i_frequency*100.0/n_frequencies + (i_run+1)*100.0/n_runs/n_frequencies
        if (i_run + 1) % 100 == 0:
            # Print an update.
            print("{:.0f} kHz, {:.0f}, {:.2f} %:"
                " {:.2f} deg, {:.2f} deg;"
                ...
                i_run,
                progress,
                np.mean(np.rad2deg(Δθ_log[i_frequency, (i_run-99):(i_run+1)])),
                # np.mean(Δθ_log[0, i_run]),
                np.mean(np.rad2deg(Δφ_log[i_frequency, (i_run-99):(i_run+1)])),
