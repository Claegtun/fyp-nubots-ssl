...
# Import the data.
data = np.loadtxt("./main_method/distance/logs/angles.csv", delimiter=',')
...
for i_distance in range(n_distances):
    θ = data[i_distance*n_runs:(1+i_distance)*n_runs,2]
    φ = data[i_distance*n_runs:(1+i_distance)*n_runs,3]
...
axs[0,0].plot([1,5], np.rad2deg([θ_true,θ_true]), "r-")
axs[0,0].plot(distances, np.rad2deg(θ_mean), "kx-")
axs[0,0].set_ylabel("Mean azimuth ($\degree$)")
axs[0,0].legend(["act.", "est."])
axs[1,0].plot(distances, np.rad2deg(θ_var), "kx-")
axs[1,0].set_ylabel("Variance of azimuth ($\degree^2$)")
axs[1,0].set_xlabel("Distance (m)")
axs[0,0].set_title("The Error in Azimuth")
axs[0,1].plot(distances, np.rad2deg(φ_mean), "kx-")
axs[0,1].plot([1,5], np.rad2deg([φ_true,φ_true]), "r-")
axs[0,1].set_ylabel("Mean elevation ($\degree$)")
axs[1,1].plot(distances, np.rad2deg(φ_var), "kx-")
axs[1,1].set_ylabel("Variance of elevation ($\degree^2$)")
axs[1,1].set_xlabel("Distance (m)")
axs[0,1].set_title("The Error in Elevation")
plt.suptitle("The Error against Distance with a Clap")
fig.set_figheight(5)
fig.set_figwidth(8)
plt.subplots_adjust(wspace = 0.28)
plt.savefig("./main_method/distance/plots.png", dpi = 1024)
plt.show()