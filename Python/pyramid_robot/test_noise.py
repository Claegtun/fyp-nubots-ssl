...
def compute_reverb_weighting(room):
    """
...
def compute_tdoa(x_0, x_1, f_s):
    """
    @brief  Computes the time-difference of arrival (TDOA) from two frames from 
            two microphones by GCC-PHAT.
...
for i_noise in range(n_noises):
    ...
    for i_run in range(n_runs):
        ...
        try:
            r = estimate_position_from_all(x, f_s)
        except OverflowError as error:
